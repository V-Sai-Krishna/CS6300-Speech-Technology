...
np.random.seed(42)
def log_sum(*array) : 
	return reduce(lambda x,y: log_add(x, y), array)
...
class HMM : 
	def __init__(self, number_symbols, number_states) : 
		...
		self.A = np.random.random((number_states, number_states))
	...
	def train(self, symbol_training_instance) : 
		...
		for t in range(symbol_training_instance.shape[1]-1) :
			...
			for i in range(self.number_states) : 
				for j in range(self.number_states) : 
					eta[i,j,t] = alpha[i,t] + self.log_A[i,j] + beta[j,t+1] + self.log_B[int(symbol_training_instance[0,t+1]),j] 		
		...
		for i in range(self.number_states) : 
			for j in range(self.number_states) : 
				...
				self.log_A[i,j] = np.max(eta[i,j,:]) + np.log(numerator) - np.max(gamma[i,:]) - np.log(denominator)	 			
